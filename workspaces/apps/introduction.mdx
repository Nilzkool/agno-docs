...
Apps are tools like `FastAPI`, `PgVector`, `Streamlit`, `Jupyter`, `Django` that we define as python classes and run using `ag ws up`.
...
Copy the following contents to a file `resources.py` and run `ag ws up`
...
db_url=vector_db.get_db_connection_string()
```
