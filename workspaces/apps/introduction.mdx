...
from agno.docker.app.postgres import PostgresDb
from agno.docker.resources import DockerResources

# -*- PgVector running on port 5432:5432
vector_db = PostgresDb(pg_user="ai", pg_password="ai", pg_database="ai")
...
- Note how `PostgresDb` sets the required settings and creates the volume.
...
db_url = f"{vector_db.get_db_driver()}://{vector_db.get_db_user()}:{vector_db.get_db_password()}@{vector_db.get_db_host()}:{vector_db.get_db_port()}/{vector_db.get_db_database()}"
```

Checkout some example apps you can run on docker:

- [PgVector](/cookbook/agent_concepts/knowledge/vector_dbs)

Defining **Applications as Code** offers many benefits, such as:

- [Install requirements on startup](/workspaces/apps/features#install-requirements-on-startup)
