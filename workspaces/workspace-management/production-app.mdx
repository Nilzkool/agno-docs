...
```bash terminal
ag ws up --env prd --infra docker --type image --pull
```

```bash shorthand
ag ws up -e prd -i docker -t image -p
```
...
```bash terminal
ag ws up --env prd --infra docker --type image --force --pull
```

```bash shorthand
ag ws up -e prd -i docker -t image -f -p
```
...
```bash Build Images
ag ws up prd:docker --pull
```

```bash Force Build Images
ag ws up prd:docker -f -p
```
