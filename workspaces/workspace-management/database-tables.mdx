...
Your ECS tasks are already enabled with SSH access. This is controlled by the `ecs_exec_access=True` setting in the `AwsApp` configuration, which ensures the necessary permissions and service-level configurations for `aws ecs execute-command`. To manually migrate the production database, SSH into the production container and run the alembic command:
