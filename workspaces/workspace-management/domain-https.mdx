...
<Note>
The `ag start` command creates new resources.
</Note>
...
```bash terminal
ag start --env prd --infra aws --name listener
```
...
<Note>
The `ag patch` command updates existing resources.
</Note>
...
```bash terminal
ag patch --env prd --infra aws --name listener
```
