...
Run `ag ws config` to view the active workspace configuration
...
Remove workspace entries from Agno without deleting the physical files

<CodeGroup>

```bash Remove active workspace
ag ws delete
```

```bash Remove all workspaces
ag ws delete -a
```

```bash Delete by workspace name
ag ws delete -ws myapp
```
