...
<Note>
The `patch` command updates existing resources.
To force a new deployment with no service definition changes (e.g., to pick up a new image with the same tag), use the `--force` flag.
...
<Note>Run `ag ws up --help` or `ag start --help` to view all options for respective commands.</Note>
<Note>Command filtering can be provided using a positional `RESOURCE_FILTER` or using individual flags.</Note>
### Environment (`--env`)
...
```bash shorthand
ag start dev:docker:app::container
```
...
Use the `--pull` or `-p` flag to force pull images from the registry.
