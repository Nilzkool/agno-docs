...
Run `ag ws create` to create a new workspace, the command will ask your for a starter template, workspace name or URL.
...
```bash Create Agent App by template
ag ws create -t agent-app   # uses default agent-app template
```

```bash Create Agent App by URL
ag ws create --url https://github.com/agno-agi/agent-app-aws -n my-app
```
...
The `patch` command is under development for some resources. Use `restart` if needed
...
Use the `--pull` or `-p` flag to pull images where applicable before creating resources.
