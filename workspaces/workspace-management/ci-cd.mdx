...
Whenever a PR is opened against the `main` branch, a validation workflow runs that ensures:

1.  Code changes are formatted using `ruff`.
2.  Selected unit tests pass.
3.  The changes don't have any typing or linting errors.

This validation is configured in the `.github/workflows/test.yml` file for general pushes and pull requests, and `.github/workflows/test_on_release.yml` for release branches, where additional integration tests might be configured.
