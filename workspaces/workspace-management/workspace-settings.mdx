...
When `aws_subnet_ids` is not explicitly set, `aws_public_subnets` and `aws_private_subnets` will be combined to form the `aws_subnet_ids`. If `aws_subnet_ids` is provided, `aws_public_subnets` and `aws_private_subnets` will be ignored.
