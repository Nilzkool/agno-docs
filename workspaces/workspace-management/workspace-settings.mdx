...
Setting `build_images=True` will build images locally when running `ag ws up dev:docker --pull` or `ag ws up prd:docker --pull`.

Checkout the `dev_image` in `workspace/dev_resources.py` and `prd_image` in `workspace/prd_resources.py` to see how its used.

## Push Images

Setting `push_images=True` will push images after building when running `ag ws up dev:docker --pull` or `ag ws up prd:docker --pull`.
