...
Copy the following code to a file `resources.py` and run `ag ws start resources.py` to create a bucket called `my-bucket-885`.
...
Make sure to delete the bucket using `ag ws stop resources.py`

### Secret Manager

Copy the following code to a file `resources.py` and run `ag ws start resources.py` to create a secret called `my-secret`.
...
Delete the secret using `ag ws stop resources.py`