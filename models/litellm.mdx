...
1. **Direct SDK integration** - Using the LiteLLM Python SDK ([details](#sdk-integration))
2. **Proxy Server integration** - Using an OpenAI-compatible proxy ([docs](./litellm_openai.mdx))
...
# For proxy integration, install extras:
...
| ----------------- | --------------------- | ---------------------------------------------------- | ------- |
| `id`              | str                   | Model identifier (e.g., "gpt-4o" or a HF model)    | "gpt-4o" |
| `name`            | str                   | Display name for the model                          | "LiteLLM" |
| `provider`        | str                   | Provider name                                        | "LiteLLM" |
| `api_key`         | Optional[str]         | API key (falls back to `LITELLM_API_KEY`)            | None    |
