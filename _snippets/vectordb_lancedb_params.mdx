...
| `on_bad_vectors`  | `str`    | None    | Action to take on bad vectors ("error", "drop", "fill", "null").                                               |
| `fill_value`      | `float`  | None    | Value to fill for bad vectors when `on_bad_vectors` is "fill".                                                        |