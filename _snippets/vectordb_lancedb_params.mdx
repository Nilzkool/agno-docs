...
| `on_bad_vectors`  | `str`          | `None`  | Behaviour when encountering bad vectors: one of "error", "drop", "fill", or "null".                            |
| `fill_value`      | `float`        | `None`  | Value to fill vectors when `on_bad_vectors` is `"fill"`.                                                             |