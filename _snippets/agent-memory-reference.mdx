...
| `memories`          | `Optional[Dict[str, Dict[str, UserMemory]]]`                | Initial user memories                                      | `None`  |
| `summaries`         | `Optional[Dict[str, Dict[str, SessionSummary]]]`            | Initial session summaries                                  | `None`  |
| `runs`              | `Optional[Dict[str, List[Union[RunResponse, TeamRunResponse]]]]` | Initial runs memory for sessions                       | `None`  |
| `debug_mode`        | `bool`                                                      | Whether to enable debug logging                             | `False` |
| `delete_memories`   | `bool`                                                      | Whether to delete memories in memory tasks                  | `False` |
| `clear_memories`    | `bool`                                                      | Whether to clear all memories in memory tasks               | `False` |
...
| `add_user_memory`       | Adds a new memory and returns the memory ID       | `memory: UserMemory, user_id: Optional[str] = None`                                                                                             | `str`                 |
| `replace_user_memory`   | Updates an existing memory and returns the ID     | `memory_id: str, memory: UserMemory, user_id: Optional[str] = None`                                                                             | `str`                 |
...
| `acreate_user_memories` | Creates memories from messages (async)            | `message: Optional[str] = None, messages: Optional[List[Message]] = None, user_id: Optional[str] = None`                                          | `str`                 |
| `search_user_memories`  | Searches memories using specified retrieval method | `query: Optional[str] = None, limit: Optional[int] = None, retrieval_method: Optional[Literal["last_n", "first_n", "agentic"]] = None, user_id: Optional[str] = None` | `List[UserMemory]`    |
...
| `create_session_summary`   | Creates a summary for a session from stored session runs       | `session_id: str, user_id: Optional[str] = None`      |
| `acreate_session_summary`  | Creates a summary for a session from runs (async)              | `session_id: str, user_id: Optional[str] = None`      |
...
| `memory_capture_instructions`| `Optional[str]`             | Custom memory capture instructions for the manager            | `None`  |
