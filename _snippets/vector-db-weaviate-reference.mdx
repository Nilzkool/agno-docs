> **Note:** Weaviate integration uses the Weaviate V4 client and supports asynchronous operations.
...
| `client` | `Optional[weaviate.WeaviateClient]` | `None` | Pre-configured synchronous Weaviate client instance. |
...
| `embedder` | `Optional[Embedder]` | `OpenAIEmbedder()` | Embedder to use for generating vector embeddings. |
