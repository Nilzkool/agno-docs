...
| `response_modalities` | `Optional[list[str]]` | `None` | Defines the modality of the response. Can be "Text" and/or "Image". |
| `speech_config` | `Optional[dict[str, Any]]` | `None` | Configuration for speech synthesis. |
| `cached_content` | `Optional[Any]` | `None` | Content to be cached for reuse. |
| `response_format` | `Optional[Type[BaseModel]]` | `None` | Specifies the desired structured output format using Pydantic models. |
| `supports_native_structured_outputs` | `bool` | `True` | Indicates whether the model natively supports structured outputs (e.g., Pydantic models). |
