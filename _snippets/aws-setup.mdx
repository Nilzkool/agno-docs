...
<Steps>
  ...
  <Step title="Update region and subnets">
    Update the `aws_region` and add your subnet IDs to the `workspace/settings.py` file (required for ECS services)
    ...
    ws_settings = WorkspaceSettings(
        ...
        # or specify public/private subnets separately
        aws_public_subnets=["subnet-xyz", "subnet-xyz"],
        aws_private_subnets=["subnet-abc", "subnet-abc"],
        ...
    )
    ```
