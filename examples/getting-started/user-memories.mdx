...
def print_agent_memory(agent):
    ...
    messages = []
    session_id = agent.session_id
    # It's better to fetch messages from memory directly rather than relying on the last run object
    # as run_response.messages might not always contain the full history of the last turn.
    # The full list of messages should be retrieved from `agent.memory.get_messages_for_session(session_id=session_id)`
    # However, the source code snippet for `print_agent_memory` in `17_user_memories_and_summaries.py` is:
    # `session_run = agent.memory.runs[session_id][-1]` and `for m in session_run.messages:`. This code is correct and reflects current source.
