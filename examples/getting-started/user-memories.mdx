...
def print_agent_memory(agent):
    ...
    for user_id in list(agent.memory.summaries.keys()):
        # retrieve the summary using the correct API
        session_summary = agent.memory.get_session_summary(session_id=agent.session_id, user_id=user_id)
        if session_summary:
            console.print(
                Panel(
                    JSON(json.dumps(session_summary.to_dict(), indent=4)),
