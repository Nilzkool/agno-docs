...
For scheduling logic, create `scheduler.py` and `workflow.py` in the new code directory `cookbook/workflows/content_creator_workflow`. Update references accordingly to underscore path rather than hyphens.
...
<Steps>
  ...
  <Step title="Run the workflow">
    ```bash
    python cookbook/workflows/content_creator_workflow/workflow.py
    ```
  </Step>
