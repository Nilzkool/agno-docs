...
The complete code is available in the [Agno repository](https://github.com/Nilzkool/agno).
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    pip install -r examples/apps/sql_agent/requirements.txt
    ```
  </Step>

  <Step title="Run PgVector">
    First, install [Docker Desktop](https://docs.docker.com/).
  ...
  <Step title="Load F1 data">
    ```bash
    python examples/apps/sql_agent/load_f1_data.py
    ```
  </Step>
  ...
  <Step title="Load knowledge base">
    ...
    ```bash
    python examples/apps/sql_agent/load_knowledge.py
    ```
    Pro tips for enhancing the knowledge base:
    - Add `table_rules` and `column_rules` to guide the Agent on query formats
    - Add sample queries to `examples/apps/sql_agent/knowledge_base/sample_queries.sql`
  </Step>
  ...
  <Step title="Launch the app">
    ```bash
    streamlit run examples/apps/sql_agent/app.py
    ```
