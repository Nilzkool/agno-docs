...
This example shows how to build Sage, a Perplexity-like Answer Engine that intelligently determines whether to perform a web search or conduct a deep analysis using Agno's built-in KnowledgeTools and ReasoningTools based on the user's query.

Sage:
1. Uses real-time web search (DuckDuckGo) and deep contextual analysis using Agno's KnowledgeTools and ReasoningTools to provide comprehensive answers
...
- Deep contextual analysis using Agno's KnowledgeTools and ReasoningTools
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    pip install -r examples/apps/answer_engine/requirements.txt
    ```
  </Step>
  ...
  <Step title="Launch the app">
    ```bash
    streamlit run examples/apps/answer_engine/app.py
    ```
