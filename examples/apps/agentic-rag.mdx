...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    pip install -r examples/apps/agentic_rag/requirements.txt
    ```
  </Step>
  ...
  <Step title="Run PgVector">
    ...
    Then start a PostgreSQL with the pgvector extension:
    ```bash
    docker run -d \
      -e POSTGRES_DB=ai \
      -e POSTGRES_USER=ai \
  ...
  <Step title="Launch the app">
    ```bash
    streamlit run examples/apps/agentic_rag/app.py
    ```
