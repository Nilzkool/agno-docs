...
We can give our agent access to the knowledge base by setting `search_knowledge=True` to add a `search_knowledge_base()` tool to the Agent. `search_knowledge` is `True` **by default** if you add `knowledge` to an Agent. The agent can then choose to use this tool when it determines it needs to search the knowledge base for information.
