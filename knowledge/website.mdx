...
### Specific Behaviors
- When `recreate=False`, the `load()` and `aload()` methods check if a website URL already exists in the vector database using its name and skips processing it.
- Setting `skip_existing=True` in `load()` or `aload()` will also prevent re-inserting documents with the same content.
