...
data_dir = Path(__file__).parent.joinpath("data", "paul_graham")
if data_dir.is_dir():
...
- The `load()` method in `LlamaIndexKnowledgeBase` currently does nothing if no `loader` is provided. If a `loader` is provided, it calls the `loader`.
