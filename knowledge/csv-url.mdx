...
| `reader`  | `CSVUrlReader` | `CSVUrlReader()` | A `CSVUrlReader` that reads a CSV from URL and converts it into `Document`s |

`CSVUrlKnowledgeBase` is a subclass of the [AgentKnowledge](/reference/knowledge/base) class and has access to the same params.

## Developer Resources

- View [Cookbook](https://github.com/Nilzkool/agno/blob/main/cookbook/agent_concepts/knowledge/csv_url_kb.py)
