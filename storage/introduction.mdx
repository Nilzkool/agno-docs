...
Adding storage to an Agent, Team or Workflow is as simple as providing a `Storage` driver and Agno handles the rest. This is achieved by setting the `mode` parameter in the `Storage` driver, which defaults to `agent`. You can use Sqlite, Postgres, Mongo or any other database you want.
...
agent = Agent(
    storage=SqliteStorage(
        table_name="agent_sessions", db_file="tmp/data.db", auto_upgrade_schema=True, mode="agent"
    ),
