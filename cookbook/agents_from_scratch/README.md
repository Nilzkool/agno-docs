...
You may also run the Agents in the CLI, which is useful for testing and debugging. Note that with the latest updates, the CLI output for Agent responses now includes enhanced streaming of intermediate steps (like reasoning and tool calls) for a more detailed real-time view.
