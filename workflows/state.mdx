...
For example, you can use the `SqliteStorage` to cache results in a Sqlite database.
...
Similarly, you can use `YamlStorage` or `JsonStorage` by importing from `agno.storage.yaml` and `agno.storage.json`, respectively, and providing `mode="workflow"`.
