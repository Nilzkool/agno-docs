...
We typically evaluate Agents on 3 dimensions:
...
Performance evals measure the latency and memory footprint of Agent operations.

<Note>
While latency will be dominated by the model API response time, we should still keep performance top of mind and track agent performance with and without certain components. E.g., it would be good to know what's the average latency with and without storage, memory, a new prompt, or a new model.
</Note>
