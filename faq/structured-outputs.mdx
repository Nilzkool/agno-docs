...
Some model classes **do not support Structured Outputs**, or you may want to fall back to JSON mode even when the model supports both options. In such cases, you can enable **JSON mode** by setting `use_json_mode=True`. Additionally, for backward compatibility you can disable structured outputs by passing `structured_outputs=False`, which has the same effect as `use_json_mode=True`.

Agno agents will also automatically fall back to JSON mode when the selected model does not natively support Structured Outputs.
...
agent = Agent(
    ...
    description="You are a helpful assistant that can extract information from a user's profile.",
    response_model=User,
    use_json_mode=True,  # or structured_outputs=False
)
```
