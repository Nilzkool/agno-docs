...
The recommended way to configure `MCPTools` or `MultiMCPTools` is to use the `command` parameter. This parameter accepts a string or a list of strings that represent the command to run the MCP server (e.g., `"npx -y @modelcontextprotocol/server-filesystem"` or `["npx", "-y", "@modelcontextprotocol/server-filesystem"]`).
