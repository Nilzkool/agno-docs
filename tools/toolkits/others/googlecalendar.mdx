...
| `create_event` | Create a new event in the user's primary calendar. Parameters include `title` (Optional[str]), `description` (Optional[str]), `location` (Optional[str]), `start_datetime` (str), `end_datetime` (str), `timezone` (Optional[str]), `attendees` (List[str] = []), `send_updates` (Optional[str] = "all"). |
