...
agent.print_response("Get the last 10 messages from the channel #random-junk", markdown=True)
...
| --------------------- | ------ | ------- | ------------------------------------------------------------------- |
| `token`               | `Optional[str]`  | `None`       | Slack API token for authentication. If not provided, will check SLACK_TOKEN environment variable.                                  |
