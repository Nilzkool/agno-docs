...
| `get_emails_by_thread`| `bool`  | `True`  | Enable retrieving emails from a specific thread           |
...
| --------------------- | ---------------------------------------------------------- |
| `get_latest_emails`   | Get the latest X emails from the user's inbox with ID, thread ID, in-reply-to, and references              |
| `get_emails_from_user`| Get X number of emails from a specific sender with ID, thread ID, in-reply-to, and references             |
| `get_unread_emails`   | Get the latest X unread emails with ID, thread ID, in-reply-to, and references                           |
| `get_starred_emails`  | Get X number of starred emails with ID, thread ID, in-reply-to, and references                           |
| `get_emails_by_context`| Get X number of emails matching a specific context with ID, thread ID, in-reply-to, and references       |
| `get_emails_by_date`  | Get emails within a specific date range with ID, thread ID, in-reply-to, and references                  |
| `get_emails_by_thread`| Retrieve all emails from a specific thread with their full details including ID, thread ID, in-reply-to, and references |
| `create_draft_email`  | Create and save an email draft                            |
| `send_email`          | Send an email immediately                                 |
| `send_email_reply`    | Respond to an existing email thread by message ID and thread ID, ensuring the subject is prefixed with "Re:" and returns sent email details including ID. | 
