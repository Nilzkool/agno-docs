...
| `scrape`  | `bool`      | `True`  | Enables the scraping functionality. If `crawl` is also `True`, `scrape` will be set to `False`. Default is `True`.          |
| `crawl`   | `bool`      | `False` | Enables the crawling functionality. If `scrape` is `False`, `crawl` will be set to `True`. Default is `False`.         |
