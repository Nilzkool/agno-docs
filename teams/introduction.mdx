...
team_with_memory = Team(
    ...
    num_history_runs=5, # Changed from num_of_interactions_from_history
)
...
<Steps>
  <Step title="Create HackerNews Team">
    ...
    import asyncio # Added import for asyncio as it's used with aprint_response
    ...
    from agno.tools.newspaper4k import Newspaper4kTools # Added import for Newspaper4kTools
    ...
    # Run the team
    report = asyncio.run(hackernews_team.arun(
        "What are the top stories on hackernews?", stream=False # Changed to arun for async, specify stream=False
    ))

    # The report object from aprint_response is the full RunResponse, not just content
    # To get the content for printing, we access report.content
    print(f"Title: {report.content.title}") # Access content.title
    print(f"Summary: {report.content.summary}") # Access content.summary
    print(f"Reference Links: {report.content.reference_links}") # Access content.reference_links
    ```
