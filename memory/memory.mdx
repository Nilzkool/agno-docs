...
When we speak about Memory, the commonly agreed upon understanding of Memory is the ability to store insights and facts about the user the Agent (or Team) is interacting with. In short, build a persona of the user, learn about their preferences and use that to personalize the Agent's (or Team's) response.

The `Memory` class, and its associated features like `enable_agentic_memory` and `enable_user_memories`, are fully supported by both the `Agent` and `Team` classes in Agno.
...
### Custom Memory Creation (with `MemoryManager`)

For fine-grained control over what kind of information is stored as memories, you can use the `MemoryManager` class. This allows you to provide specific instructions to the underlying model that processes and extracts memories from conversations.

This is particularly useful when you want to focus on certain types of information (e.g., academic interests only, or technical details) and ignore others (e.g., personal preferences, names, hobbies).

```python custom_memory_creation.py
from agno.memory.v2 import Memory
from agno.memory.v2.db.sqlite import SqliteMemoryDb
...
# Using default memory manager for comparison
memory = Memory(model=Gemini(id="gemini-2.0-flash-exp"), db=memory_db)
jane_doe_id = "jane_doe@example.com"
