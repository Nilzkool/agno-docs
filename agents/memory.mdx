...
- `enable_agentic_memory=True` gives the Agent a tool to manage memories of the user, this tool passes the task to the `Memory` class (v2). You may also set `enable_user_memories=True` which always runs the `Memory` class (v2) after each user message.
...
- We can set `add_history_to_messages=True` and `num_history_runs=3` to add the messages from the last 3 runs automatically to every message sent to the agent.
...
| `num_history_runs` | `int` | `3` | Number of historical runs to add to the messages. |
| `enable_user_memories` | `bool` | `False` | If true, creates and stores personalized memories for the user, and automatically updates them after each run. |
| `enable_session_summaries` | `bool` | `False` | If true, creates and stores session summaries, and automatically updates them after each run. |
| `enable_agentic_memory` | `bool` | `False` | If true, enables the agent to manage the user's memory via a tool. |
