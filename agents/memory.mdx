...
Memory gives an Agent the ability to recall relevant information. Memory is a part of the Agent's context that helps it provide the best, most personalized response.
...
|----------------------------|---------|-----------|------------------------------------------------------------------------------------------------------------------------|
| `memory`                   | `Memory`| `Memory()`| Agent's memory object used for storing and retrieving information (user memories & session summaries).                |
...
| `enable_user_memories`     | `bool`  | `False`   | If true, create and store personalized memories for the user after each run.                                           |
| `enable_session_summaries` | `bool`  | `False`   | If true, create and store session summaries after each run.                                                            |
| `enable_agentic_memory`    | `bool`  | `False`   | If true, enables the agent to manage the user's memories via tools.                                                   |
| `read_chat_history`        | `bool`  | `False`   | If true, adds a `get_chat_history()` tool to the Agent to read the full session history on demand.                    |
| `read_tool_call_history`   | `bool`  | `False`   | If true, adds a `get_tool_call_history()` tool to the Agent to inspect past tool calls.                                |
