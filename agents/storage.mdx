...
Adding storage to an Agent, Team or Workflow is as simple as providing a `Storage` driver and Agno handles the rest. You can use Sqlite, Postgres, JSON or YAML file storage, or any other database you want.

### Filesystem-based storage drivers

Agno also ships with two file-based storage drivers:
...
# JSON storage
json_storage = JsonStorage(dir_path="tmp/sessions", mode="agent")

# YAML storage
yaml_storage = YamlStorage(dir_path="tmp/sessions", mode="agent")
```
...
The first time you run this, the answer to "What was my last question?" will not be available. But run it again and the Agent will able to answer properly, because we have fixed the session id and the session is persisted across runs.
