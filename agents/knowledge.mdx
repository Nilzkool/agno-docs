...
| `search_knowledge`   | `bool`                                       | `True`  | Adds a tool that allows the Model to search the knowledge base (Agentic RAG).                       |
| `add_references`     | `bool`                                       | `False` | Enable RAG by adding references from the knowledge base to the user prompt.                        |
| `retriever`          | `Callable[..., Optional[list[dict]]]`        | `None`  | Custom function for context retrieval when `add_references=True`.                                  |
| `references_format`  | `Literal['json','yaml']`                     | `json`  | Format for returned references from `search_knowledge_base()`.                                      |
| `context`            | `Dict[str,Any]`                              | `None`  | Static context provided to the Agent.                                                               |
| `add_context`        | `bool`                                       | `False` | If `True`, add `context` into the user prompt.                                                     |
