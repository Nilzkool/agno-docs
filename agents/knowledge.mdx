...
| `retriever`          | `Callable[..., Optional[list[dict]]]`    | `None`  | Function to get context to add to the user message when `add_references` is True.            |
| `references_format`  | `Literal['json', 'yaml']`               | `json`  | Specifies the format for knowledge references in the user prompt.                             |
