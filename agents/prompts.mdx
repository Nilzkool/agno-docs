...
| `timezone_identifier`         | `str`                             | `None`   | Timezone identifier (e.g. `Etc/UTC`) used when adding datetime.                                      |
| `add_name_to_instructions`    | `bool`                            | `False`  | If True, add the Agent's name to the instructions list in the system message.                       |
| `add_state_in_messages`       | `bool`                            | `False`  | If True, format both system and user messages using session state variables.                        |
| `system_message`              | `str`                             | `None`   | Provide the system prompt directly as a string.                                                      |
...
The Agent creates a default user message, which is either the input message or a message with the context (if `add_context=True`). The default user message can be customized using:

| Parameter                   | Type                       | Default  | Description                                                                                           |
|-----------------------------|----------------------------|----------|-------------------------------------------------------------------------------------------------------|
| `context`                   | `Dict[str, Any]`           | `None`   | Additional context (a dict) to add to the user message.                                               |
| `add_context`               | `bool`                     | `False`  | If True, add the context to the user prompt.                                                         |
| `resolve_context`           | `bool`                     | `True`   | If True, call any functions in the context before adding it to the prompt.                           |
| `add_references`            | `bool`                     | `False`  | If True, enable RAG by adding knowledge base references to the user message.                         |
| `retriever`                 | `Callable`                 | `None`   | Function to fetch references when `add_references` is True.                                          |
| `references_format`         | `"json"  "yaml"` | `"json"`| Format of the inserted references.                                                                   |
| `add_history_to_messages`   | `bool`                     | `False`  | If True, include chat history in messages sent to the model.                                          |
| `num_history_responses`     | `int`                      | `3`      | Number of historical assistant responses to include.                                                  |
| `user_message`              | `Union[List, Dict, str]`   | `None`   | Provide the user prompt directly as a string or structure.                                           |
| `user_message_role`         | `str`                      | `user`   | Role for the user message.                                                                           |
| `create_default_user_message`| `bool`                    | `True`   | If True, build a default user prompt using context, references and history.                          |
