...
The Agent creates a default user message, which is either the input message or a message with the `context` if `add_references=True`. The default user message can be customized using:

| Parameter                 | Type                        | Default   | Description                                                                                                              |
|---------------------------|-----------------------------|-----------|--------------------------------------------------------------------------------------------------------------------------|
| `context`                 | `Dict[str, Any]`            | `None`    | Additional context added to the end of the user message.                                                                 |
...
| `add_references`          | `bool`                      | `False`   | Enable retrieval-augmented generation by adding references from the knowledge base to the prompt.                       |
| `retriever`               | `Callable`                  | `None`    | Function to get references to add to the `user_message`. Called when `add_references` is True.                          |
...
| `num_history_responses`   | `int`                       | `None`    | (Deprecated) Number of historical responses to add. Use `num_history_runs` instead.                                       |
| `num_history_runs`        | `int`                       | `3`       | Number of historical runs to include in the messages. Overrides `num_history_responses`.                                  |
| `user_message`            | `Union[List, Dict, str]`    | `None`    | Provide the user prompt as a string. Note: this will ignore the message passed to the run function.                      |
