...
| `instructions`               | `List[str]`              | `None`    | List of instructions added to the system prompt in `<instructions>` tags. Default instructions are also created depending on values for other settings. |
...
| `timezone_identifier`        | `str`                    | `None`    | Timezone identifier (e.g. `Etc/UTC`) for generating the current datetime.                                                         |
| `add_name_to_instructions`   | `bool`                   | `False`   | If True, add the agent's name to the instructions.                                                                                 |
| `add_state_in_messages`      | `bool`                   | `False`   | If True, format messages with session state variables.                                                                             |
...
The Agent creates a default user message, which is either the input message or a message augmented with context, references, or history depending on settings. The default user message can be customized using:

| Parameter                 | Type                       | Default   | Description                                                                                                   |
|---------------------------|----------------------------|-----------|---------------------------------------------------------------------------------------------------------------|
| `context`                 | `Dict[str, any]`           | `None`    | Additional context available for the prompt functions.                                                        |
| `add_context`             | `bool`                     | `False`   | If True, add the `context` to the user prompt.                                                                |
| `resolve_context`         | `bool`                     | `True`    | If True, resolve any callables in the `context` before formatting.                                            |
| `add_references`          | `bool`                     | `False`   | Enable RAG by adding references from the knowledge base to the prompt.                                        |
| `retriever`               | `Callable`                 | `None`    | Function to get references to add to the user message when `add_references=True`.                             |
| `references_format`       | `Literal["json", "yaml"]` | `"json"` | Format of the references inserted in the user message.                                                        |
| `add_history_to_messages` | `bool`                     | `False`   | If True, add chat history messages to the list sent to the model.                                              |
| `num_history_runs`        | `int`                      | `3`       | Number of historical runs to include in the messages (replaces `num_history_responses`).                       |
| `user_message`            | `Union[List, Dict, str]`   | `None`    | Provide the user prompt as a string or Message. This overrides other user message formatting settings.          |
