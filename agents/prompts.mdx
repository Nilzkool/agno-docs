...
| `add_datetime_to_instructions` | `bool` | `False` | If True, add the current datetime to the prompt to give the agent a sense of time. This allows for relative times like "tomorrow" to be used in the prompt. Can also specify `timezone_identifier` for custom timezones. |
| `add_state_in_messages` | `bool` | `False` | If True, session state variables and resolved context variables are available for use in the system and user messages using `{key}` syntax. |
