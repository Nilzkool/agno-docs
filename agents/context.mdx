...
|-----------|------|---------|-------------|
| `context` | `Dict[str, Any]` | `None` | A dictionary of functions or values that are resolved before the agent runs, acting as dependency injection for Agents. |
| `add_context` | `bool` | `False` | If True, resolved context is added to the user message. |
| `resolve_context` | `bool` | `True` | If True, functions in the context are called and their results are used. |
