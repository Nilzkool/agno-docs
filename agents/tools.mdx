...
## Using the Knowledge Toolkit

Agno includes a built-in toolkit for knowledge-based reasoning and retrieval: `KnowledgeTools`. It wraps an `AgentKnowledge` instance and provides `think`, `search`, and `analyze` tools to iterate through an internal reasoning loop.
...
# Build or load your knowledge base
kb = AgentKnowledge.from_documents([...])

# Initialize the toolkit (enable think, search, analyze as needed)
...
# Ask a question; the agent will cycle: Think → Search → Analyze
agent.print_response(
    "What are the main recommendations for mild hypertension?", stream=True
)
```

This toolkit helps you:

- **Think**: Brainstorm keywords or plan your search strategy (internal only).
- **Search**: Query your knowledge base for relevant documents.
- **Analyze**: Verify the relevance and completeness of returned documents.

For more details, see the [Knowledge Tools guide](/tools/knowledge).
