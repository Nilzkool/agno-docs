...
|-----------|------|---------|-------------|
| `tools` | `List[Union[Toolkit, Callable, Dict, Function]]` | - | A list of tools provided to the Model. Tools are functions the model may generate JSON inputs for. |
| `show_tool_calls` | `bool` | `True` | Print the signature of the tool calls in the Model response. |
